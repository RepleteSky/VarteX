        ...
        for (inp, out, lead_times, variables, out_variables) in self.dataset:
            ...
            for i in range(inp.shape[0]):
                if self.region_info is not None:
                    region = get_random_region_info(self.lat, self.lon, self.crop_size, self.patch_size)
                    yield self.transforms(inp[i]), self.output_transforms(out[i]), lead_times[i], variables, out_variables, region
                else:
