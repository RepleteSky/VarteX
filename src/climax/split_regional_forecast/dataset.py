...
class IndividualForecastDataIter(IterableDataset):
    def __init__(self, dataset, transforms: torch.nn.Module, lat, lon, crop_size, patch_size, output_transforms: torch.nn.Module, region_info = None):
        super().__init__()
        self.dataset = dataset
        ...
        for (inp, out, lead_times, variables, out_variables) in self.dataset:
            ...
            for i in range(inp.shape[0]):
                if self.region_info is not None:
                    if i % 4 == 0:
                        region_info = get_region_info("lt_region", self.lat, self.lon, self.patch_size)
                    elif i % 4 == 1:
                        region_info = get_region_info("rt_region", self.lat, self.lon, self.patch_size)
                    elif i % 4 == 2:
                        region_info = get_region_info("lb_region", self.lat, self.lon, self.patch_size)
                    elif i % 4 == 3:
                        region_info = get_region_info("rb_region", self.lat, self.lon, self.patch_size)
                    yield self.transforms(inp[i]), self.output_transforms(out[i]), lead_times[i], variables, out_variables, region_info
                else:
