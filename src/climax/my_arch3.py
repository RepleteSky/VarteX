...
from climax.encoder_block import MyBlock

from .parallelpatchembed import ParallelVarPatchEmbed


class ClimaX3(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                MyBlock(
                    ...
                    # drop=drop_rate, # timm のバージョン違い
                )
                for i in range(self.depth)
            ]
        )
        self.norm = nn.ModuleList([nn.LayerNorm(self.representative_dim) for _ in range(self.depth)])
